...
const DailyView: React.FC<any> = ({ logs, messages, onAddLog, onAddMessage }) => {
  ...
  const handleImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = async () => {
      ...
      } catch (err) { onAddMessage({ role: 'ai', text: "이미지 분석에 실패했습니다." }); }
  ...
  const handleChat = async (e: React.FormEvent) => {
    ...
    setChatInput('');
    onAddMessage({ role: 'user', text: inputText });
    setChatLoading(true);
    try {
      const res = await processChatMessage(inputText, JSON.stringify(todayLogs));
    ...
    <div className="flex flex-col h-full bg-slate-50">
      <div className="p-4 bg-white border-b">
        <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl p-5 text-white shadow-lg">
          <p className="text-xs opacity-80 font-bold uppercase tracking-wider">오늘 단백질</p>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-40">
        {messages.map((msg:any) => (
          <div key={msg.id} className={`flex ${msg.role === 'ai' ? 'justify-start' : 'justify-end'}`}>
            <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm ${msg.role === 'ai' ? 'bg-white border text-slate-800' : 'bg-indigo-600 text-white'}`}>
              {msg.text || (msg.logId && "식사 기록이 추가되었습니다.")}
              {msg.logId && <div className="mt-2 text-xs font-bold border-t pt-1">✅ 기록됨</div>}
            </div>
          </div>
        ))}
        <div ref={chatEndRef} />
      ...
      <div className="absolute bottom-0 w-full p-4 bg-white border-t">
        <form onSubmit={handleChat} className="flex gap-2 items-center">
          ...
          <button type="submit" className="text-indigo-600 font-bold px-2">전송</button>
